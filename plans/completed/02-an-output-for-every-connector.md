...
connection. So one new monitor ends every session on the machine.
...
configures the head from the section written for it at startup.
...
action on the card's device, with `HOTPLUG=1` in its environment. A
numeric `CONNECTOR=` id is present when the kernel can name the
connector. The
DRM subsystem puts `HOTPLUG=1` in the broadcast itself; it is stored
nowhere in sysfs.

**The design drops the `no-output` taint.** Its whole reason was
"this connector has no routing until a restart". With routing complete
from startup, a dark connector needs only the honest taint,
`display.liken.sh/disconnected`, which is unchanged. The design drops
the refusal in `NodePrepareResources` with it.

## What unplug now costs

Delivering hotplug events changes unplug as well. Without them, no
unplug ever reaches Weston. With them, Weston destroys
...
screen may stay dark before the pod is evicted. An unplug affects
only the one screen whose cable moved.
...
* **`force-on=true` on every section.** Weston enables a forced head
  ...
  config would have to hard-code a fixed modeline, and Weston never
  re-modes a live head. The source logs "Detected a monitor change
  ... not bothering to do anything about it." A hard-coded mode
  stays wrong for every monitor whose native mode differs.
...
* **Running udevd.** udevd is a whole device manager, where the need
  is one netlink group name, and liken's init already proves the
  kernel group alone is enough.
...
* **A different compositor.** cage serves one app on one output by
  design. The wlroots family subscribes to hotplug through libudev
  the same way Weston does, so no event arrives there either, and a
  desktop's worth of config comes with it. Mutter requires logind and
  a session bus. The
...
2. **A monitor arrives, nothing else moves, passed.** The operator
   ...
   seconds. The Bluetooth operator's drills showed the same. The
   plug-in ran the whole chain inside one second: the
...
4. **A reseated cable, passed.** A five second unplug and replug put
   the taint on and took it off. Weston disabled and re-enabled
   ...
   reseat shorter than the toleration evicts nothing.
