...
listening socket and returns or closes its descriptor. That call
belongs in libwayland upstream. Until it exists, the only way the
module can stop accepting on a socket is to unlink the path.
...
module binds has no lock file, so that `listen` is no longer
refused.
