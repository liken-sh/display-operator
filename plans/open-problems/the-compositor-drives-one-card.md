...
scheduler gives the pod one card, and the pod publishes that card's
connectors and nothing else.

The audio operator does not have this limit, and the difference is the
daemon. One PipeWire spans every ALSA card on a machine, so the audio
...
`<node>-display.liken.sh`, and the conflict-retry loop would make them
...
* Distinct driver names per card, one slice each. This splits the
  ...
  consumer must know which card's driver to claim. It is more
  machinery for less clarity.

## What is not measured

No liken machine has two graphics cards today. liken-1 has one
integrated GPU. So the cost of this limit has not been seen, and the
