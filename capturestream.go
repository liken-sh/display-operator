...
// the knobs are checked against them, the encoder starts, and only
// then does the status go out. A denial after a 200 has nowhere to
// go.
func (s *captureServer) stream(w http.ResponseWriter, r *http.Request,
	...
	go func() {
		frames <- s.feed(r.Context(), session, encoder, first, chosen, origin, still, mediaType)
	}()
	...
	if feedErr != nil {
		...
		s.reportMidStream(connector, feedErr)
		return nil
	}
