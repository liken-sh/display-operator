// The docs domain is its own module, nested under the operator's.
// Hugo builds the site, and Hugo's dependency graph is hundreds of
// modules the operator never imports. A nested module keeps that
// graph in this go.sum, so the operator's go.sum stays the short
// list of what the driver actually links.
//
// The tool directive is the pin: `go tool hugo` builds Hugo at
// exactly this version, from source, on any machine with Go. No
// separate Hugo install, and no drift between a workstation's Hugo
// and CI's. This is the standard Hugo build, pure Go, because the
